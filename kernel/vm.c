...
void superuvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  uint64 a;
  pte_t *pte;
  int sz;
...
void superuvmfree(pagetable_t pagetable, uint64 supersz)
{
  if (supersz > 0)
...
int superuvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
...
#ifdef LAB_PGTBL
void vm_recur_print(pagetable_t pagetable, int depth);
void vmprint(pagetable_t pagetable)
{
  printf("page table %p\n", (void *)pagetable);
  vm_recur_print(pagetable, 1);
}
...
void vm_recur_print(pagetable_t pagetable, int depth)
{
  for (int i = 0; i < 512; i++)
  {
    ...
    if (pte & PTE_V)
    {
      ...
      uint64 pa = PTE2PA(pte);
      printf("%p: pte %p pa %p\n",
             (void *)(pagetable + i), (void *)pte, (void *)pa);
      // 打印自己的页表项指向的页表
      if ((pte & (PTE_R | PTE_W | PTE_X)) == 0)
      {
        // 指向低一级的页表,权限必须为0，有效必须为1
        uint64 child = PTE2PA(pte);
        vm_recur_print((pagetable_t)child, depth + 1);
      }
    }
  }
}
